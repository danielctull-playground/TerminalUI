...
struct ByteEventTests {
  ...
  private struct Small: ByteEvent, Equatable {
    ...
    init(parsing bytes: [Byte]) throws {
      guard let byte = bytes.first, byte < 0x44 else { throw Failure() }
      self.byte = byte
      try checkTrailingBytes(bytes.dropFirst())
    }
  }
  ...
  private struct Unbound: ByteEvent, Equatable {
    ...
    init(parsing bytes: [Byte]) throws {
      guard let byte = bytes.first else { throw Failure() }
      self.byte = byte
      try checkTrailingBytes(bytes.dropFirst())
    }
  }
  ...
  private struct Large: ByteEvent, Equatable {
    ...
    init(parsing bytes: [Byte]) throws {
      guard let byte = bytes.first, byte > 0x46 else { throw Failure() }
      self.byte = byte
      try checkTrailingBytes(bytes.dropFirst())
    }
  }
